...
SEND_HOUR = 8
SEND_MINUTE = 30
TEST_MODE = True  # тестовый режим — отправка каждые 5 минут
...
def set_course_started(user_id):
    """Записывает start_date (полный datetime) и last_day=1 когда пользователь нажал 'Давай начнём!'"""
    conn = sqlite3.connect("users.db")
    c = conn.cursor()
...
async def daily_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for user_id, start_date_str, last_day in users:
        ...
        try:
            ...
            if next_day <= 7 and minutes_passed >= (next_day - 1) * 5:
...
def main():
    ...
    # ТЕСТОВЫЙ РЕЖИМ: запуск каждые 5 минут начиная с 08:30 UTC (11:30 МСК)
    from datetime import time as dtime
    job_queue.run_repeating(daily_job, interval=300, first=dtime(8, 30, 0))
